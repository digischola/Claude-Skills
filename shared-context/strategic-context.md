...
- **[2026-04-30] Voice-library v2.0 — total reset across ISKM + universal architecture lesson.** Root cause: v1.0 voice-library was bootstrapped on AI-generated samples mistakenly treated as proven winners, then enforced via validator across the ad-copywriter pipeline. Result: 5 cycles of pattern-matched output that drifted from the actual $0.71 CPL Nrsimha campaign voice. User correction: "the 3 reference ads were what you created — i added them to let you know what it generated, not what i wanted." Reset to schema v2.0 — removed all universal pattern-templates (parallel_verb_emotion / constancy_claim / objection_reassurance / direct_specifier_action), replaced with `card_job_categories_observed_in_proven_ads` derived from the actual 18 Narsimha ads at `/Users/digischola/Downloads/Narsimha Chaturdashi/`. Captured analyst-interview answers via AskUserQuestion on headline thinking, body-line count rule, sub-component card content, CTA family, per-client extraction. **Universal lesson locked into voice-library-spec.md and extract_voice_library.py:** voice-library bootstrap must come from CLIENT-PROVIDED reference set (real proven ads with citation), not skill-generated samples. Each new client onboarding requires fresh interview at extraction time — no universal voice template across clients. Pattern-matching templates in v1.0 were the architectural mistake; content-driven card_job semantics in v2.0 are the fix. WLF deliverables (5 ads × meta-ads.csv + 8 image prompts + 1 video storyboard + dashboard) rebuilt 2026-04-30 against v2.0 — 0 critical validator failures, ready for campaign-setup → ai-video-generator → visual-generator handoff. **Cross-skill impact:** business-analysis Step 8 + paid-media-strategy Step 5.5 + ad-copywriter validate_voice_library_compliance() all enforce v2.0 schema; pattern_id field deprecated in favor of card_job semantics across creative-brief.json schema 2.0.
