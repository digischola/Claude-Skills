...
def load_pillars_status(brand_folder: Path):
    p = brand_folder / "brand" / "pillars.md"
    if not p.exists():
...
def render_calendar_markdown(week_start: date, slots, gap_alerts,
    ...
    lines.append("---")
    lines.append(f"Generated by content-calendar skill. Source: brand/idea-bank.json. Pillars: LOCKED. Re-run after capturing more entries to fill gaps.")
