...
| --- | --- | --- |
| `Desktop/Digischola/brand/idea-bank.json` | work-capture, trend-research, content-calendar, post-writer, case-study-generator, performance-review | Source of truth for every piece of planned/written content. |
| `Desktop/Digischola/brand/pillars.md` | personal-brand-dna (writes), every downstream skill (reads, gates pre-check) | LOCKED after approval. Hard gate in `append_to_idea_bank.py`. |
| `Desktop/Digischola/brand/voice-guide.md` | post-writer, repurpose, case-study-generator | Read during draft generation. |
| `Desktop/Digischola/brand/brand-identity.md` | visual-generator, scheduler-publisher, content-calendar | Palette, fonts, easing — LOCKED. |
| `Desktop/Digischola/brand/credentials.md`, `credential-usage-log.json` | post-writer | 30-day rotation lookup. |
| `Desktop/Digischola/brand/channel-playbook.md`, `icp.md`, `brand-wiki.md`, `wiki-config.json` | All downstream personal-brand skills | LOCKED brand wiki. |
| `Desktop/Digischola/brand/queue/pending-approval/*.md` where `posting_status ≠ posted` | scheduler-publisher, repurpose | Active drafts. Never touch. |
| `Desktop/Digischola/brand/queue/published/*.md` (<180 days) | performance-review, scheduler-publisher | Attribution window. |
| `Desktop/Digischola/brand/performance/log.json` | performance-review | Append-only scoring history. |
| `Desktop/Digischola/brand/performance/*.md` | performance-review, weekly-ritual | Weekly review reports. |
| `Desktop/Digischola/brand/weekly-ritual.state.json` | weekly-ritual | Idempotency state. |
| `Desktop/Digischola/brand/calendars/*.md` | content-calendar, scheduler-publisher, post-writer | Active planning. Never touch. |
| `Desktop/{Client}/{Project}/wiki/**` | business-analysis + every downstream client skill | Client wiki is the memory layer. |
| `Desktop/{Client}/{Project}/deliverables/**` (reports, dashboards, configs, CSVs) | Every client skill | Primary deliverables — real work product. |
...
| --- | --- | --- |
| `Desktop/Digischola/brand/_mining/**` | personal-brand-dna | Re-run `scripts/mine_transcripts.py` |
| `Desktop/Digischola/brand/_research/trends/{week}/` | trend-research | Re-run `scripts/trend_research.py prompt --week YYYY-WXX` |
| `Desktop/Digischola/brand/remotion-studio/out/**` (intermediates) | visual-generator | Re-run `scripts/generate_reel.py` for the draft |
| `Desktop/{Client}/{Project}/sources/perplexity-*.md` | market-research | Re-generate prompt + Perplexity round-trip |
| `Desktop/{Client}/{Project}/sources/*keyword*.csv` | market-research | Re-export Google Keyword Planner |
| `Desktop/{Client}/{Project}/sources/*.png` (audit screenshots) | landing-page-audit | Re-capture via Chrome MCP |
...
- **No shared state.** Weekly-ritual's `weekly-ritual.state.json` lives in `brand/`. Housekeeping's `housekeeping.state.json` lives in the skill dir. Both are PROTECTED.
