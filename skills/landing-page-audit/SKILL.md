...
Save as `{client-folder}/landing-page-audit.html` (folder root — it's a presentable).

**Multi-page collision fallback:** if a single program folder will host audits for MULTIPLE landing pages, the default short filename collides. Before writing, check whether `{client-folder}/landing-page-audit.html` already exists. If it does, fall back to `{client-folder}/{page-name}-landing-page-audit.html` (where `{page-name}` is the audit target's slug — e.g., `home-landing-page-audit.html`). Do the same for the markdown findings file in Step 5b.
...
**File:** `{client-folder}/_engine/working/audit-findings.md` (default short name). Fall back to `{client-folder}/_engine/working/{page-name}-audit-findings.md` only if the short name already exists in that working folder (multi-page collision case — same rule as Step 5).
...
- [2026-04 → CONSOLIDATED] Template/validator/process foundation (encoded into assets/templates, scripts/generate_dashboard.py, scripts/validate_output.py, references/screenshot-capture.md, references/markdown-findings-spec.md): bundled HTML templates with Chart.js UMD + dark mode + collapsibles; auto-capture via Chrome MCP (mobile 375×667 + desktop 1440×900) with computer-use fallback for Elementor; placeholder-div min-height fixes; mandatory `{{PLACEHOLDER}}` replacement check; CRO Content Rhythm & Scannability (13% weight); Persuasion Copy Density & Sectional Flow (14% weight); always-scan-Desktop for existing client folder + brand-config glob at intake; Step 5b markdown findings report; SKILL.md progressive-disclosure trim (Step 5b → references/markdown-findings-spec.md).
...
- [2026-04-29] [STRUCTURAL REFACTOR] All skill internals now live in `_engine/` subfolder; presentables at folder root. Default filenames dropped client/page-name prefix: `landing-page-audit.html` (root), `audit-findings.md` (`_engine/working/`); screenshots in `_engine/sources/`. **Multi-page collision exception:** when a program folder hosts MULTIPLE page audits, fall back to `{page-name}-landing-page-audit.html` + `{page-name}-audit-findings.md` to disambiguate. Validator derives the markdown companion from the HTML filename (short → short, prefixed → prefixed), accepts legacy sibling-of-HTML location as fallback, and warns when prefixed filename has no real collision. → Updated SKILL.md, references/markdown-findings-spec.md, references/skill-coordination.md, scripts/validate_output.py.
