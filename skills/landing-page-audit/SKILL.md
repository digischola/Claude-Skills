...
- [2026-04-29] [STRUCTURAL REFACTOR — filename simplification] Output filename templates dropped redundant client/page-name prefix. Default filenames are now `landing-page-audit.html` (folder root) and `audit-findings.md` (in `_engine/working/`) — folder location already encodes client + program. **Multi-page collision exception:** if a single program folder hosts audits for MULTIPLE landing pages, fall back to `{page-name}-landing-page-audit.html` and `{page-name}-audit-findings.md` to disambiguate (e.g. `Thrive Retreat/Thrive Retreat/` with home + booking + workshop audits). Validator now derives the markdown companion from the HTML filename (short → short, prefixed → prefixed), accepts the legacy sibling-of-HTML location as backwards-compat fallback, and warns when the HTML is still using the legacy `{page-name}-` prefix without a real collision. → Updated SKILL.md Step 5 + Step 5b, references/markdown-findings-spec.md, references/skill-coordination.md, scripts/validate_output.py.
