...
description: "Comprehensive pre-marketing market-research skill. Generates a Perplexity deep research mega-prompt covering 10 research dimensions (market size, Porter's, PESTEL, SWOT, competitors, keywords/audience, benchmarks, buyer personas, referral ecosystem, blue ocean), collects platform keyword data (Google Ads Keyword Planner and/or Meta Ads via Chrome tools or CSV import), analyzes with strict accuracy protocol, and produces a markdown report plus a premium HTML dashboard with Chart.js visualizations, tooltips, collapsibles, and client branding. SCOPE BOUNDARY: research only — does NOT prescribe campaign structure, budget allocation, ad copy angles, or phased plans (those belong to paid-media-strategy, downstream). Use when user mentions: new client research, market research, competitor analysis, audience research, pre-campaign research, market sizing, industry benchmarks, competitive landscape, or provides a business URL to understand before running ads. Do NOT trigger for: paid-media strategy / campaign architecture / budget split (use paid-media-strategy), ad copy writing, campaign creation, content calendars, performance reporting."
---

# Market Research

Pre-marketing research substrate for any business before a single marketing dollar is spent. Covers 10 research dimensions, applies strict accuracy protocol, and produces client-ready deliverables that paid-media-strategy then converts into a campaign plan.

**SCOPE BOUNDARY (load-bearing).** This skill produces market understanding — TAM, competitive forces, regulatory context, audience sizing, benchmarks, personas, ecosystem, blue-ocean wedges. It does NOT recommend campaign structure, budget allocation, audience tiers, ad copy angles, or phased execution. Those are the **paid-media-strategy** skill's job. End of each research dimension may carry a one-line "Marketing Implication" if the takeaway is sharp; anything more belongs downstream.
...
**Determine platform focus:** Ask which platform(s) the client needs — Google Ads only, Meta Ads only, or both. This shapes the Perplexity prompt (sections 6, 7) and the platform data step.
...
- **FRESH mode** — generate the full 10-dimension mega-prompt from scratch with NO carry-forward from prior runs or sibling programs. FRESH means what the word says: clean-slate research, every dimension researched in this run, no inherited summaries, no "DO NOT RE-RESEARCH" markers. Use FRESH when (a) no prior `_engine/sources/perplexity-*.md` exists for this program, (b) prior research is older than 6 months, (c) confidence is MEDIUM/LOW on >3 dimensions, or (d) the user explicitly says "fresh run". For multi-program clients with a sibling program already researched: that sibling's strategy.md is reference material the analyst can read, but the FRESH prompt must NOT pre-load Perplexity with carry-forward summaries — the model researches every dimension from scratch and the analyst reconciles overlaps at synthesis time.
...
Never skip Step 2 by judgment. The mode decision is the step. The earlier "FRESH program-targeted with carry-forward" sub-flavor (2026-04-29 learning) was demoted 2026-04-30 — it conflicted with the user's intent for FRESH = no inherited context. Carry-forward across programs lives in UPDATE mode (when an upstream correction explicitly authorizes partial re-research), not in FRESH.
...
Read `references/perplexity-prompt-template.md` for the mega-prompt framework. Read `references/research-dimensions.md` for all 10 dimensions:
...
**SCOPE BOUNDARY (load-bearing):** market-research ends at Dimension 10. Strategic recommendations — campaign structure, budget allocation, audience tier definitions, phased plan, ad copy angles, conversion-event recommendations, landing-page strategy — are the **paid-media-strategy** skill's job, downstream. Do NOT generate Strategic Recommendations as an 11th dimension. The Perplexity prompt should research the market, not prescribe a campaign plan. The dashboard's strategy timeline section is a **forward-pointer** ("paid-media-strategy will use the data above to build the plan"), not a built-out plan. This boundary was reasserted 2026-04-30 after scope-creep audit.
...
**If Perplexity output is weak** (fewer than 7 of 10 dimensions have substantive data): generate 1-2 targeted follow-up queries for the weakest dimensions. Maximum 2 follow-up rounds, then proceed with gaps marked BLANK.
...
- [2026-04-30] [SCOPE BOUNDARY — Dimension 11 was scope creep, removed] User flagged on the WLF re-onboard run: "market research was to do market research only and not provide strategic recommendations — that was the job of next skills. I guess I made it clear when I created skill, can you check." Audit confirmed it: the 11th "Strategic Recommendations for Ad Platform(s)" dimension covered campaign structure, budget allocation, ad copy angles, conversion actions, phased plan — all explicitly the job of `paid-media-strategy`, the next skill. The boundary had eroded over many iterations because every learning entry kept reinforcing "Section 11 must do X" instead of asking "should Section 11 exist at all". → **Patched 2026-04-30 (5 files):** (1) `SKILL.md` frontmatter description rewritten to "10 dimensions" + explicit "SCOPE BOUNDARY: research only — does NOT prescribe campaign structure, budget allocation, ad copy angles, or phased plans"; line-72 dimensions list dropped Dim 11; new SCOPE BOUNDARY paragraph added below. (2) `references/research-dimensions.md` removed §11; replaced with explicit SCOPE BOUNDARY section + reframed cross-reference table to point at downstream paid-media-strategy. (3) `references/report-structure.md` removed §11 from TOC + body; added "Handoff to paid-media-strategy" section as the report's terminal forward-pointer. (4) `references/perplexity-prompt-template.md` replaced §11 with explicit "DO NOT GENERATE STRATEGIC RECOMMENDATIONS" instruction to Perplexity. (5) `scripts/validate_output.py` dimensions-coverage check dropped from 11 to 10; the "Strategic Recommendations" regex line removed with a SCOPE BOUNDARY comment. **RULE:** market-research stops at Dimension 10 (Blue Ocean). End of each dimension may carry a one-line "Marketing Implication" — anything more belongs to paid-media-strategy. When future learning entries propose "Section 11 must do X", the answer is "Section 11 doesn't exist" — propose the change to paid-media-strategy instead.

- [2026-04-30] [FRESH MODE — no carry-forward, ever] User clarified on the WLF re-onboard: "when I mean fresh run no context of previous run should be there." The 2026-04-29 "FRESH program-targeted with carry-forward from sibling program" learning entry (validated on this same WLF program one day prior) directly conflicted with this. That pattern was demoted today. **Patched 2026-04-30:** Step 2 FRESH-mode definition rewritten — "FRESH means what the word says: clean-slate research, every dimension researched in this run, no inherited summaries, no 'DO NOT RE-RESEARCH' markers". For multi-program clients, the sibling program's strategy.md is reference material the analyst can read at synthesis time, but the FRESH Perplexity prompt MUST NOT pre-load with carry-forward. Carry-forward (when explicitly authorized by an upstream correction) lives in UPDATE mode only. **RULE:** FRESH = clean slate, no inheritance. UPDATE = targeted reconcile after upstream correction. NO-NEW-RESEARCH = no Perplexity output, log reason. The three modes are now non-overlapping. The token-saving carry-forward optimization is real but it is NOT FRESH; it is UPDATE.
...
- [2026-04-27 → PARTIALLY SUPERSEDED 2026-04-30] [Self-brand · DigiSchola productized-audit India] Two patterns to carry forward. (1) When the audit subject is itself a marketing-services brand selling the audited capability, research surfaces a credibility paradox that doubles as creative material — DigiSchola sells "tracking from day one" but its own site has zero pixels. The Phase 0 fix is also the strongest LinkedIn-content moment ("I just installed the pixel after auditing my own site"). (2) For productized SKUs at micro-budget (₹500/day = ₹3,500/week), Meta Purchase-event learning-phase exit requires ~₹75K/week — about 21× available spend. The upper-funnel-proxy-event recommendation pattern is real, but it now belongs to **paid-media-strategy** (Section 11 of this skill no longer exists per 2026-04-30 scope reset). Pattern (1) — credibility paradox as creative material — surfaces in Dimension 4 SWOT (weakness → creative material) within market-research; pattern (2) — phase-1 proxy event + phase-2 purchase transition — moves to paid-media-strategy. **RULE migrated:** the budget-vs-event-volume math should appear in market-research's Dimension 7 benchmarks as a calculated finding ("at SGD X/day vs Singapore CPM, weekly conversion volume = N events"); the recommendation that flows from it (which event to optimize, when to transition) is paid-media-strategy's call.
...
- [2026-04-29] [ISKM WLF — brief correction at start of Step 2 worth flagging as a pattern] User explicitly flagged a brief error at the start of the market-research run, BEFORE the Perplexity prompt was generated, and asked for the brief to be corrected. (Original brief logged "first batch starts May 9" as the spend kickoff date; user corrected to "marketing live May 1, first served weekend May 9-10, always-on weekly indefinitely.") The correction is exactly the kind of thing that should land in `briefs.md` as an in-place edit (per Same-Client Re-Run Rule) — NOT as a new ACTIVE entry. Then the Perplexity prompt is generated against the CORRECTED brief and `_engine/wiki/log.md` records the correction with reason. Confirmed: re-runs and corrections are logged-not-versioned. **RULE:** When a client correction lands at the start of market-research that affects a downstream input (timeline, budget, goal, audience), update the `briefs.md` ACTIVE entry in place + log the correction in `_engine/wiki/log.md` BEFORE generating the Perplexity prompt. Never generate a Perplexity prompt against an outdated brief — the prompt's strategic recommendations would inherit the error.
- [2026-04-29] [Dashboard validator — strategic-frameworks regex] `validate_output.py` `strategic_frameworks_present` check expects literal "Porter", "PESTEL", and "SWOT" tokens visible in the dashboard HTML. WLF dashboard had SWOT (full grid) but only carry-forward references to Porter's + PESTEL in section copy — failed CRITICAL on first run (1/3 frameworks). Fix: added Porter's + PESTEL collapsibles in the Market Size section of the dashboard, with summary content drawn from the carry-forward shared-market research. Validator passed 3/3 after that. **RULE:** Even when carry-forward dimensions are referenced rather than re-researched, the dashboard MUST surface Porter / PESTEL / SWOT as visibly-named sections (collapsibles are fine) so the validator regex passes. The carry-forward content can be summary-level + reference the prior program's strategy.md for full depth — but the literal tokens must appear in the deliverable.
- [2026-04-29] [STRUCTURAL REFACTOR] All skill internals (wiki, sources, working, configs) now live in `_engine/` subfolder; presentables (HTML/PDF/CSV/MP4) at folder root. Multi-program `_shared/` collapsed into client-root `_engine/`. Output filenames dropped redundant client/business-name prefix — `research-dashboard.html`, `market-research.md` (folder encodes client + program). Validators accept legacy `{client}-`prefixed names as fallback. → Updated SKILL.md, references/, scripts/, evals/.
