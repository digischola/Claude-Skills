...
**Why this over API:** Google Ads API requires MCC account, developer token, Basic access approval (multi-day wait), OAuth setup. Keyword Planner CSV export works with any Google Ads account, takes 2 minutes per batch, zero setup.

**Google Ads API (optional, if credentials exist):** Run `scripts/google_keyword_volume.py` if user has completed `references/google-ads-api-setup.md` setup with Basic access token. Faster for repeat runs but requires one-time credential setup.
