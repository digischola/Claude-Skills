...
**Voice-library inheritance (MANDATORY — bedrock):** Read `{client}/_engine/wiki/voice-library.json` (or `{client-root}/_engine/wiki/voice-library.json` for multi-program). Populate creative-brief.json with:

1. **Root-level `event_facts`** (when campaign type = event/festival/recurring program): `{"canonical_string": "...", "campaign_type": "event", "must_appear_on_every_variant": true}`. The canonical string comes from offerings.md + wiki facts (day / time / place / free-or-price / language). This is what every ad-copywriter variant MUST include verbatim.
2. **Per-creative `voice_anchor`**: pick a `pattern_id` from voice-library.headline_patterns matching the creative's persona + audience tier + emotional register. E.g. `{"pattern_id": "objection_reassurance", "rationale": "Cold cultural-affinity audience — objection-question pattern matches"}`. ad-copywriter validator enforces compliance.
3. **Per-creative `cta_verb_noun_pair`**: pick from voice-library.cta_verb_noun_pairs based on the visual subject of the creative. E.g. food card → `{"verb": "Reserve", "noun": "a plate"}`; class card → `{"verb": "Reserve", "noun": "a class spot"}`. Lazy reuse of the same noun across all carousel cards is forbidden.

If voice-library.json is missing → block: business-analysis Step 8 must run first. See `~/.claude/shared-context/voice-library-spec.md`.
...
-->
- [2026-04-30] [BEDROCK — voice-library inheritance into creative-brief.json] Step 5.5 expanded: creative-brief.json now MUST carry root-level `event_facts.canonical_string` (event campaigns) + per-creative `voice_anchor.pattern_id` (from voice-library.headline_patterns) + per-creative `cta_verb_noun_pair` (from voice-library.cta_verb_noun_pairs). Solves voice-drift across audience-segmented carousels (WLF Apr-30: 5 cards / 5 different trust-line strings / generic CTAs). Spec: `~/.claude/shared-context/voice-library-spec.md`. ad-copywriter `validate_voice_library_compliance()` validates the brief→copy chain. **RULE:** No creative-brief.json ships without event_facts (event campaigns) + voice_anchor + cta_verb_noun_pair populated from voice-library.json. If voice-library.json is missing → block; business-analysis Step 8 must run first.
- [2026-04-10/11 → CONSOLIDATED] SaaS-app + general foundation (encoded into Step 2/3, references/strategy-frameworks.md, scripts/validate_output.py, scripts/run_evals.py, assets/template-*.html, evals/evals.json): creative-readiness gate (zero creative = launch blocker, mandate production timeline); Phase-1 unprofitability framing for subscription apps; learning-phase viability calculator (budget ÷ CPA = weekly events vs 50-event threshold); data-supported/directional labels on all phase criteria; for-app-installs always ask creative-readiness + growth-priority; failure/fallback protocols on intake; run_evals.py output-check + structure-check; {{PLACEHOLDER}} CRITICAL check + Chart.js UMD verification; HTML templates with dark mode/collapsibles/tooltips/copy-buttons; eval cases 4-5 (wellness retreat Meta-only, temple dual-objective).
...
- [2026-04-29] [ISKM Weekend Love Feast — multi-program retention follow-on] Successfully ran paid-media-strategy as a downstream consumer of a fully-populated market-research wiki (sibling Nṛsiṁha program already established Singapore Hindu market context; WLF program added recurring-retention frame). Four client-decision overrides at intake meaningfully reshaped the strategy from market-research's recommendations: (a) Phase 0 zero-spend (overrode "directional light warmup ~SGD 35-40"), (b) weekly review cadence (overrode "bi-weekly recommended"), (c) sponsor-layer deferred to 60-day review, (d) fresh creative production starting with Windsor.ai Nṛsiṁha pull. All four captured as `[INFERRED — extending brief]` in the Brief Alignment section per skill anti-over-architecting rule. **RULE:** When client overrides at paid-media-strategy intake conflict with directional recommendations from upstream market-research, the override wins as long as the strategy explicitly flags it in Brief Alignment. The market-research `[Directional]` label is what makes this safe — directional recommendations are explicitly opinion-not-data, so client preference legitimately replaces them.
- [2026-04-29] [Validator nuance — `[EXTRACTED]` substring vs market-research's regex] paid-media-strategy's `validate_output.py` uses **strict substring match** (`content.count("[EXTRACTED]")`) for source labels, NOT the regex pattern from market-research's [2026-04-27] patch. Result: report had 17 source-label tags total but validator only counted 4 because most were citation-suffixed forms like `[EXTRACTED — Perplexity 2026-04-29]` or `[Data-supported — Perplexity [43]]`. Required adding bare-form `[EXTRACTED]` and `[INFERRED]` tokens specifically (separately from any descriptive suffix). **RULE:** When writing the paid-media-strategy report, use bare-form `[EXTRACTED]` and `[INFERRED]` tokens (no suffix) on at least 10 data points, even if you also use descriptive forms elsewhere. Patch candidate (medium priority): port market-research's regex pattern to paid-media-strategy's validator so citation-suffixed forms count too. Until patched, conform to the strict substring match.
- [2026-04-29] [Dashboard validator — class-name tokens] `validate_output.py` checks for very specific class-name tokens that don't match HTML5-semantic dashboards: tooltips need literal `class="tip"` (not `.kpi-tooltip` or `[title=]`); copy buttons need literal `<button class="copy-btn">` or `copyText(`; collapsibles need `collapse-btn` or `collapse-body` class names (not just `<details>` / `<summary>`). HTML5-semantic dashboards built without these specific class names fail validator warnings even when fully functional. **RULE:** When writing dashboards for paid-media-strategy specifically, use the validator's expected class names (`tip`, `copy-btn`, `collapse-btn`, `collapse-body`) alongside any other styling classes — they don't conflict. Patch candidate (low priority): broaden validator to accept `<details>`/`<summary>` for collapsibles, `[title=]` or `[data-tooltip=]` attributes for tooltips, and any clipboard-API-call onclick for copy buttons.
