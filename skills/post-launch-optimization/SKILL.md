...
- `shared-context/output-structure.md` — write final HTML dashboards/PDFs to the client/program folder root; intermediate MD/JSON to `_engine/working/`
...
If client has a wiki folder (`{client-folder}/_engine/wiki/`), read:
- `_engine/wiki/strategy.md` — campaign architecture, targets, personas
- `_engine/wiki/index.md` — what skills have been run
- `_engine/working/*-optimization-report.md` — previous analysis (session memory, Layer 11)
...
Update `{client-folder}/_engine/wiki/log.md` — add OPTIMIZATION entry with date and key findings.
